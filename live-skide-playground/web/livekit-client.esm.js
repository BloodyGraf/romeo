...
const ParticipantInfo = {
    ...
    fromJSON(object) {
        return {
            ...
            permission: isSet$1(object.permission)
...
const ClientConfiguration = {
    ...
    fromJSON(object) {
        return {
            ...
            resumeConnection: isSet$1(object.resumeConnection)
...
const VideoConfiguration = {
    ...
    fromJSON(object) {
        return {
            hardwareEncoder: isSet$1(object.hardwareEncoder)
            ...
            senders.forEach((sender) => {
                ...
                if (sender.track === mediaStreamTrack) {
                    try {
                        ...
                        livekitLogger.warn('failed to remove track', { error: e, method: 'unpublishTrack' });
                    }
                }
            });
        }
...
function canPassThroughQueue(req) {
    const canPass = Object.keys(req).find((key) => passThroughQueueSignals.includes(key)) !==
        undefined;
...
class SignalClient {
    ...
    async sendRequest(req, fromQueue = false) {
...
class RTCEngine extends events.exports.EventEmitter {
    constructor() {
        ...
        this.handleDataChannel = async ({ channel }) => {
            if (!channel) {
                return;
            }
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    if (recoverable) {
                        ...
                        livekitLogger.info(`could not recover connection after ${maxReconnectRetries} attempts, ${duration}ms. giving up`);
...
class Room extends events.exports.EventEmitter {
    ...
    constructor(options) {
        ...
        this.handleRestarted = async (joinResponse) => {
            livekitLogger.debug(`reconnected to server region ${joinResponse.serverRegion}`);
            this.state = RoomState.Connected;
        ...
        setTimeout(async () => {
            ...
            if (publishAudio && publishVideo) {
                try {
                    ...
                    livekitLogger.warn('received error while creating media', { error: errKind });
